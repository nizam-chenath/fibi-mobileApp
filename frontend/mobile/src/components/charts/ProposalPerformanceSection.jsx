...
// components/charts/AwardedProposalsSection.jsx
import React, { useMemo, useEffect, useRef, useState } from 'react';
import { View, Text, StyleSheet, Animated, Easing } from 'react-native';
import Svg, { Path } from 'react-native-svg';
import useTheme from '../../hooks/useTheme.jsx';
  ...
  return (
    <View style={styles.section}>
      <View style={styles.card}>
        <Text style={styles.cardLabel}>{incomeLabel.toUpperCase()}</Text>
        <Text style={styles.cardValue}>{formattedIncome}</Text>
              ...
              <View key={entry.id} style={styles.barWrapper}>
                <View
                  style={[
                    styles.bar,
                    {
                      backgroundColor: entry.color || theme.colors.secondary,
      ...
      <View style={styles.card}>
        <Text style={styles.cardLabel}>{donutLabel.toUpperCase()}</Text>
        <View style={styles.donutWrapper}>
          {normalizedSegments.length > 0 ? (
            <>
              <Svg width={DONUT_SIZE} height={DONUT_SIZE}>
                {normalizedSegments.map((segment) => {
                  ...
                  return (
                    <Path
                      ...
                      fill={segment.color || theme.colors.secondary}
                    />
                  );
                })}
              </Svg>
              <View style={styles.donutCenter}>
                <Text style={styles.centerLabel}>{donutLabel.toUpperCase()}</Text>
                <Text style={styles.centerValue}>{formattedDonut}</Text>
              </View>
            </>
          ) : (
